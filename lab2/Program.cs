...
namespace lab2
{
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            Console.WriteLine($"\t {rn[0].ToString()} + {rn[1].ToString()} = {RomanNumber.Add(rn[0], rn[1]).ToString()}");

            Console.WriteLine($"\n\nРазность:\t {number[1]} - {number[0]} = {number[1] - number[0]}");
            Console.WriteLine($"\t\t {rn[1].ToString()} - {rn[0].ToString()} = {RomanNumber.Sub(rn[1], rn[0]).ToString()}");

            Console.WriteLine($"\n\nУмножение:\t {number[0]} * {number[1]} = {number[0] * number[1]}");
            Console.WriteLine($"\t\t {rn[0].ToString()} * {rn[1].ToString()} = {RomanNumber.Mul(rn[1], rn[0]).ToString()}");

            Console.WriteLine($"\n\nДеление:\t {number[1]} / {number[0]} = {number[1] / number[0]}");
            Console.WriteLine($"\t\t {rn[1].ToString()} / {rn[0].ToString()} = {RomanNumber.Div(rn[1], rn[0]).ToString()}");
